...
- **Border:** Web containers use Soft Rule; Android cards take a 1 dp `outlineVariant` hairline on the low surface role, with one edge treatment per card and never a border under a shadow.
...
The landing hero depicts the device with CSS geometry rather than decorative imagery: a softly squared enclosure with a hairline and a lit screen showing the reader, a breathing status LED beside the silkscreen specs, a USB-C slot and two screws along the bottom edge, the "Free, forever" card at its upper right, and faint circular guides behind it. Its colours resolve to the landing palette through tokens scoped to `.device-scene`, the enclosure follows the site theme, and the bezel, status LED and connector keep their own material colours.

What glows on the glass is the reader itself, not a drawing of it: the markup is the firmware's (`header`/`.header-inner`, `.brand`/`.brand-mark`, `.search`, `.library-shelf`, `.library-group`, `.active-label`, `.library-articles`, `.search-library`, `.list`, `.storage-status`, `.storage-meter`, `.pack-list`, `.inline-action`, `.manager-section`, `.upload`, `.article-nav`, `.download-row`) and the styles are the reader's own tokens and values copied from `assets/style.css`, scoped under `.screen`, so the preview and the board cannot drift apart. Four of the reader's pages are live inside it (`Your libraries`, `Search`, `Manage library` and an article), navigated by the shelf, the header's Manage link, the search field and the back links.

Search is the device's, not a mock: titles are `tl_normalize`d (trim, collapse, ASCII case-fold) and matched as prefixes, the exact hit sorts first and renders bold, results are grouped per library and capped at twenty, and the page reports the same counts and empty states the firmware writes. The libraries, titles, lead sections and shelf counts are the shipped ones; the storage figures come from the ESP32-C3 `packs` partition and the published catalogue. The guide group lists the first rows a phone would show, and the pack group lists the three articles this preview bundles. Every row and every search result opens, because a preview with dead ends is worse than a short one.

The glass is a phone-width viewport, so the reader's own `≤680px` header layout is written out directly instead of through a media query, the fluid `h1` is pinned to the `26px` its clamp resolves to on a phone, and the landing page's inherited base rules (centred hero text, balanced headings, antialiased glyphs, the site focus ring) are reset, because the reader is a different document. Only the vertical padding is trimmed for a hero-sized screen. Library groups open natively (`<details>`).
...
- **Structure uses timed curves.** A destination change, a setup step, and a disclosure all use Material's transitions: the outgoing side leaves in 90 ms on an accelerating curve, the incoming side arrives in 210 to 300 ms on a decelerating one. `fadeThrough()` switches peer destinations, the bottom-navigation tabs and a body that swaps between its empty state and its content, by fading out fast and fading the newcomer in from 92% scale. `sharedAxisX()` moves a setup step in the direction of travel, so a wizard has a direction the user can feel.
...
**The Sourced Numbers Rule.** Every count, capacity and price statement on the landing page comes from `packs/catalog-source.json`, the partition tables, or the host tooling. The page states no testimonials, benchmarks or commercial claims.
