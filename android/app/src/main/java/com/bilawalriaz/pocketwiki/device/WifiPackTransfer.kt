 ...
 *  - SoftAP hop: [route] joins the device's access point
 *    (`WifiNetworkSpecifier`, user confirms once per install rather than once
 *    per pack), uploads to the AP IP, and [releaseAp] then lets the network go
 *    so the phone returns to its saved one.
 *
...
class WifiPackTransfer(context: Context) {
  ...
  /** The PocketWiki access point while it is held open, with the request that
   *  keeps it. A batch of packs hops once, so the system asks the user to
   *  approve the network once instead of once per pack. */
  ...
  /** Upload over the current network to `host` (IP literal), or over
   *  [network] when one is given. */
  suspend fun uploadTo(
    host: String,
    file: File,
    name: String,
    onProgress: (Float) -> Unit,
    network: Network? = null,
  ): WifiUploadResult = withContext(Dispatchers.IO) { post(host, file, name, onProgress, network) }

  /** Where an upload to the device's access point should be routed. */
  sealed interface ApRoute {
    /** Usable. [network] is null when the phone is already on the access point
     *  and its own route reaches it. */
    data class Reachable(val network: Network?) : ApRoute
    data object Unreachable : ApRoute
  }

  /**
   * The route to the PocketWiki access point [ssid], opening it if needed and
   * keeping it open for the packs that follow. [alreadyJoined] says the phone
   * is on that network right now, which needs no request at all.
   *
   * The connection is released by [releaseAp], not here: an install that
   * covers several packs must not return the phone to its saved network
   * between them, nor ask for the same access point again.
   */
  suspend fun route(ssid: String, alreadyJoined: Boolean): ApRoute {
    apNetwork?.let { if (ssid == apSsid) return ApRoute.Reachable(it) }
    if (alreadyJoined) return ApRoute.Reachable(null)
    return join(ssid)?.let { ApRoute.Reachable(it) } ?: ApRoute.Unreachable
  }

  /** Let the access point go; Android returns the phone to its saved network. */
  fun releaseAp() {
    apCallback?.let { runCatching { connectivity.unregisterNetworkCallback(it) } }
    apCallback = null
    apNetwork = null
    apSsid = null
  }

  private suspend fun join(ssid: String): Network? {
