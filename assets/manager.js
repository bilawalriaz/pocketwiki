...
async function installFromDevice(url, name, statusEl, progress = document.getElementById("catalog-progress")) {
...
function renderCatalog(catalog) {
    ...
    catalog.packs.forEach(pack => {
        ...
        const fits = pack.bytes <= installable;
        button.textContent = fits ? (installed.has(pack.id) ? "Update" : "Install") : "Not enough space";
        button.disabled = !fits;
        if (!fits) {
            button.title = "This pack needs " + sizeText(pack.bytes) + "; " +
                sizeText(installable) + " is available.";
        }
        button.onclick = async () => {
            button.disabled = true;
            showStatus(catalogStatus, "Downloading " + pack.name + "…");
            const ok = await installFromDevice(pack.url, pack.id, catalogStatus);
...
async function syncCatalog() {
    try {
        const response = await fetch("/api/packs/catalog/sync", { method: "POST" });
        if (response.ok) return await response.json();
    } catch (error) { /* try the public catalogue below */ }
    try {
        const response = await fetch("https://packs.educated.space/index.json", { cache: "no-store" });
        if (response.ok) return await response.json();
    } catch (error) { /* fall back to the device copy below */ }
    return fetchCatalog();
}
...
async function loadCatalog(manual) {
    ...
    const catalog = manual ? await syncCatalog() : await fetchCatalog();
    ...
    if (!catalog) {
        ...
        if (manual) showStatus(catalogStatus, "Could not refresh the catalogue. Connect PocketWiki to Wi-Fi with internet access and try again.");
        return;
    }
...
async function loadStatus() {
    ...
    label.textContent = result.connected ? "Connected to " + result.ssid : "PocketWiki access point";
