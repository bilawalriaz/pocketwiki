...
export const decompress = (buf, opts = { defaultHeapSize: 1024 * 1024 }) => {
    ...
    // Some Emscripten builds expose the unsigned 64-bit content-size result
    // as zero. A bounded fallback still lets ZSTD_decompress report its real
    // output length and avoids returning an empty article.
    const size = contentSize <= 0 || contentSize > 0x7fffffff ? opts.defaultHeapSize : contentSize;
