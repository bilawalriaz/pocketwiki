...
3. The service converts the fetched sources into the existing article input,
   ...
   `.pwp` archive using the existing pack tooling. New packs should use the
   existing bounded Zstd v2 path by default: it is already the packer's default
   and gives better density. Keep gzip v1 as a compatibility and rollback path
   for older firmware or a pack that fails measured C3 resource checks.
...
need a Zstd or gzip decoder for this flow.
