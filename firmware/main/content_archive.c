 ...
 * Articles are raw DEFLATE streams with one shared trained dictionary at the
 * payload tail; anything else is refused here, before it can be installed. */
/* Why the last pack was refused, for the manager to show. Only the archive
 * layer knows the difference between "not a pack at all" and "a valid pack
 * this build cannot hold", and the two need different words. */
static const char *s_reject_reason;

const char *ca_last_reject_reason(void)
{
    return s_reject_reason != NULL ? s_reject_reason
...
static void reject(const char *reason)
{
    ...
    ESP_LOGE(TAG, "pack refused: %s", reason);
}
                                  ...
                                  uint8_t *format_version)
{
    ...
    uint8_t ver = hdr[4];
    if (ver != PW_FORMAT_VERSION) {
        ESP_LOGE(TAG, "content: unsupported version %u", ver);
        reject("That pack is an older or unknown format; rebuild it with the "
               "current packer.");
        return ESP_ERR_INVALID_VERSION;
    }
    ...
    uint32_t ps = rd32(hdr + 12);
    uint32_t doff = rd32(hdr + 16);
    uint32_t dlen = rd32(hdr + 20);
 ...
 * The index version must match the content version. */
                                 ...
                                 pw_file_ctx_t *index_ctx, content_archive_t *out)
{
    ...
    if (rd32(hdr) != PW_PACK_MAGIC || pack_ver != PW_FORMAT_VERSION) {
        return ESP_ERR_INVALID_VERSION;
    }
    uint32_t content_len = rd32(hdr + 8);
    uint32_t index_len = rd32(hdr + 12);
    if (content_len < PW_CONTENT_HEADER_SIZE || index_len < PW_INDEX_HEADER_SIZE) {
        return ESP_ERR_INVALID_SIZE;
    }
...
esp_err_t ca_read_payload(uint32_t payload_offset, uint8_t *buf, size_t len)
{
    ...
    return ca->content.read(ca->content.ctx,
                            PW_CONTENT_HEADER_SIZE + payload_offset, buf, len);
}
