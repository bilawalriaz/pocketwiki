...
#define PW_FORMAT_VERSION      2u            /* zstd frames + shared dict */
#define PW_FORMAT_VERSION_LEGACY 1u          /* gzip frames, no dict */
#define PW_CONTENT_HEADER_SIZE 32u
#define PW_CONTENT_HEADER_SIZE_LEGACY 16u
#define PW_INDEX_HEADER_SIZE   32u
#define PW_PACK_HEADER_SIZE    32u
#define PW_MAX_DICT_BYTES      32768u

/* The archive keeps the pack's raw dictionary blob (32 KiB max) in RAM.
 * The server's digested dictionary references this blob instead of copying
 * it, so the blob must remain valid until the active archive changes. */
...
typedef struct {
    ...
    uint32_t dict_offset;       /* payload-relative offset of the zstd dict */
    uint32_t dict_len;          /* dict blob size; 0 when the pack has none */
    uint8_t *dict;              /* owned dict blob; NULL when dict_len == 0 */
    ...
    uint8_t format_version;     /* PW_FORMAT_VERSION or PW_FORMAT_VERSION_LEGACY */
...
typedef struct {
    ...
    uint32_t content_offset;    /* offset of the article's frame within the
                                 * content payload (zstd v2 or gzip v1) */
    uint32_t comp_len;
    uint32_t uncomp_len;
    uint32_t crc32;
} pw_article_meta_t;
...
/* Dictionary blob for the active archive's zstd frames. Returns NULL and
 * leaves *len at 0 when the archive has no dictionary. Valid until the next
 * ca_load_pack/ca_revert_to_flash. */
