...
typedef struct {
    bool connected;
    /* True when the saved uplink was dropped because its signal could not
     * share the radio with the access point. Credentials are kept; setting
     * the Wi-Fi again from /manage clears this. */
    bool uplink_suspended;
    char ssid[33];
    char ip[16];
    int8_t rssi;
