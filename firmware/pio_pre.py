"""PlatformIO pre-build: refresh embedded CSS and the built-in guide pack."""
...
# The built-in archive: one v3 (DEFLATE + trained dictionary) pack that every
# target decodes.
subprocess.run([
    sys.executable, str(repo / "tools/pack_content.py"), "build",
    str(repo / "articles/pocketwiki"), str(out),
], check=True)
