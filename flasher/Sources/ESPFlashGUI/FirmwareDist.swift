...
enum FirmwareDistScanner {
    ...
    /// directory looking for `firmware/dist`, then fall back to the project's
    /// canonical location under the home directory.
    static func locateDefaultRoot() -> URL? {
        ...
        let homeCandidate = fm.homeDirectoryForCurrentUser
            .appendingPathComponent("esp32_oled_wifi")
            .appendingPathComponent("firmware")
            .appendingPathComponent("dist")
        if fm.fileExists(atPath: homeCandidate.path) { return homeCandidate }
        return nil
    }
