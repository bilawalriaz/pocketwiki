...
def test_firmware_reads_payload_after_the_header():
    ...
    assert "PW_CONTENT_HEADER_SIZE + payload_offset" in function
...
def test_compression_roundtrip(sample_archive):
    for e in sample_archive.entries:
        body = sample_archive.article_bytes(e)   # validates decode + len + CRC
...
def test_small_corpus_uses_small_dictionary(sample_archive):
    # A small corpus must not carry a full 32 KiB dictionary it cannot amortize:
    # every article re-reads that blob into the inflate ring, and flash stores
    # it. The starter pack trains to well under the cap.
...
def test_crc_mismatch_content(sample_archive):
    ...
    # flip the last byte of the first article's DEFLATE stream: the archive
