"""PocketWiki archive format — single source of truth for content.bin / index.bin.

See docs/ARCHIVE_FORMAT.md. All integers little-endian. The entry array is
fixed-width so the firmware can binary-search it in flash.

Every article is an independent raw DEFLATE stream compressed against one
shared trained dictionary stored at the end of the content payload. The device
loads that dictionary into the 32 KiB inflate ring it already owns before
decoding an article, so dictionary density costs no static RAM on either board.

The firmware mirrors this module in firmware/main/content_archive.c.
"""
...
FORMAT_VERSION = 3           # raw DEFLATE frames + a shared trained dictionary
...
# The 32-byte content header carries the dictionary's offset and length. The
# dictionary sits at the end of the payload so per-article offsets are
# unaffected by its presence and the device can stream one article at a time.
...
def pack_content_header(payload_size: int, dict_offset: int = 0,
    ...
    """content.bin header. The trained dictionary is stored at the END of the
    payload, so article offsets are unaffected by its presence:
...
def unpack_pack_file(data: bytes) -> tuple[bytes, bytes]:
    ...
    if content_len < CONTENT_HEADER_SIZE or index_len < INDEX_HEADER_SIZE:
...
class Archive:
    ...
    def __init__(self, content: bytes, index: bytes):
        ...
        self.dict = (content[self.payload_offset + dict_offset:
                     ...
                     if dict_len else b"")
        # The dictionary is a raw DEFLATE preset window, so it can never be
        # larger than the 32 KiB ring the device loads it into.
        if dict_len > 32768:
            raise ArchiveError(f"content.bin dict_len {dict_len} exceeds 32768")
    ...
    def article_bytes(self, e: dict) -> bytes:
        ...
        Independent raw DEFLATE streams compressed against the shared trained
        dictionary (self.dict) — the same preset-dictionary model the device
        reproduces by pre-filling its inflate ring before each article.
        """
        ...
        try:
            dobj = zlib.decompressobj(-15, zdict=self.dict if self.dict else b"")
            out = dobj.decompress(raw) + dobj.flush()
        except zlib.error as exc:
            raise ArchiveError(f"article {e['id']}: deflate decode failed: {exc}") from exc
        if dobj.unused_data or not dobj.eof:
            raise ArchiveError(f"article {e['id']}: deflate stream is not exactly one frame")
