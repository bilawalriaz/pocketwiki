...
that order, one shared dictionary trained on the article bodies, no
timestamps anywhere.

Every article is stored as a raw DEFLATE stream (format v3) compressed against
that dictionary. The device loads the dictionary into the 32 KiB inflate ring
it already owns before decoding, so dictionary density costs no static RAM.
Requires the `zstandard` Python package: its cover trainer produces the
dictionary, which DEFLATE then uses as its preset window.
"""
...
# zlib level 9 into a raw, headerless DEFLATE stream with the trained
# dictionary as the preset window. Compression level is a build-time cost only:
# decoding is the same inflate on every board.
DEFLATE_LEVEL = 9
DEFLATE_MEM_LEVEL = 9
# Trained dictionary size cap. It is also the format maximum: the device loads
# the dictionary into its 32 KiB inflate ring, so it can never be larger.
# Small corpora use a proportionally smaller dictionary (see train_dict), which
# saves both flash and the per-article dictionary read.
DICT_MAX_BYTES = 32768
DICT_MIN_BYTES = 4096
DICT_TRAIN_SAMPLES = 1500        # cap for training time on large corpora


def train_dict(bodies: list[bytes], max_bytes: int = DICT_MAX_BYTES) -> bytes:
    """Train the shared dictionary on the exact bytes the packer will compress.

    zstandard's cover trainer picks the dictionary content — it is a dictionary
    *producer*, not a frame decoder, and the bytes it returns are what DEFLATE
    uses as its preset window (the device holds the same bytes in its inflate
    ring). Deterministic for identical inputs. Returns b"" — a dict-less pack —
    when the corpus is too small to train usefully; the format supports both.

    The dictionary is sized proportionally to the corpus (total // 32, clamped
    to [DICT_MIN_BYTES, max_bytes]) so a small pack does not carry a blob it
    cannot amortize: the 100-article starter uses ~7 KiB rather than 32 KiB.
    Pass max_bytes=0 to force a dict-less pack, or an explicit byte size to
    override the adaptive choice.
    """
    samples = bodies[:DICT_TRAIN_SAMPLES]
    total = sum(len(s) for s in samples)
    if len(samples) < 8 or total < DICT_MAX_BYTES // 2:
        return b""
    if max_bytes == 0:
        return b""
    target = min(max_bytes, max(DICT_MIN_BYTES, total // 32))
    try:
...
def convert_and_rewrite(article: dict, id_map: dict,
    ...
    Articles are stored as independent raw DEFLATE streams sharing a trained
    dictionary; the ESP32 inflates each one and serves identity HTML. The page
    chrome therefore belongs inside the compressed
...
def build(articles_dir: str, out_dir: str,
          dict_bytes_max: int = DICT_MAX_BYTES) -> dict:
    ...
    # pass 2: convert, then compress each article independently. The index maps
    # an article to one raw DEFLATE stream read straight from flash, and the
    # firmware inflates it out of a 32 KiB ring pre-loaded with the pack's
    # shared dictionary.
    bodies: list[bytes] = []
    for a in articles:
    ...
    # Raw DEFLATE (wbits=-15, no zlib/gzip wrapper) with the pack's trained
    # dictionary as the preset window: the decoder must already hold the same
    # 32 KiB window, which the device guarantees by loading the blob into its
    # inflate ring before the first call.
    def compress_article(data: bytes) -> bytes:
        obj = zlib.compressobj(DEFLATE_LEVEL, zlib.DEFLATED, -15,
                               DEFLATE_MEM_LEVEL, zlib.Z_DEFAULT_STRATEGY,
                               dict_bytes)
    ...
    payload += dict_bytes
    content = af.pack_content_header(len(payload), dict_offset, len(dict_bytes)) + bytes(payload)
...
def verify_dir(out_dir: str) -> dict:
    ...
    for e in arch.entries:
        arch.article_bytes(e)  # full decode + length + CRC check
...
def main(argv=None) -> int:
    ...
    p.add_argument("--dict-bytes", type=int, default=DICT_MAX_BYTES,
                   help="max trained dictionary bytes (0=dict-less; default adapts "
                        "down for small corpora)")
    ...
    try:
        if args.cmd == "build":
            manifest = build(args.articles_dir, args.out_dir,
